...
export const useSensay = (apiKey: string) => {
  ...
  const uploadTrainingFile = useCallback(async (file: File): Promise<TrainingContent | null> => {
    ...
    try {
      ...
      if (response.data) {
        setTrainingContent(prev => [...prev, response.data as TrainingContent]);
      }
  ...
  const getConversations = useCallback(async (agentUUID?: string): Promise<Conversation[]> => {
    if (!sensayServiceRef.current || !currentUser) {
      setError('Services not initialized or user not authenticated');
      return [];
    }

    if (!agentUUID && !selectedAgent) {
      setError('No agent selected for conversation retrieval');
      return [];
    }
    ...
    try {
      ...
      const sensayService = sensayServiceRef.current;
      const targetAgentUUID = agentUUID || selectedAgent!.uuid;
      const response = await sensayService.getConversations(targetAgentUUID);
  ...
  const getConversation = useCallback(async (agentUUID: string, conversationId: string): Promise<Conversation | null> => {
    if (!sensayServiceRef.current || !currentUser) {
      setError('Services not initialized or user not authenticated');
      return null;
    }
    ...
    try {
      ...
      const sensayService = sensayServiceRef.current;
      const response = await sensayService.getConversation(agentUUID, conversationId);
  ...
  const getConversationMessages = useCallback(async (agentUUID: string, conversationId: string, pageSize = 50, cursor?: string): Promise<ChatMessage[]> => {
    if (!sensayServiceRef.current || !currentUser) {
      setError('Services not initialized or user not authenticated');
      return [];
    }
    ...
    try {
      ...
      const sensayService = sensayServiceRef.current;
      const response = await sensayService.getConversationMessages(agentUUID, conversationId, pageSize, cursor);
  ...
  const createConversation = useCallback(async (agentUUID: string, userID?: string): Promise<Conversation | null> => {
    if (!sensayServiceRef.current || !currentUser) {
      setError('Services not initialized or user not authenticated');
      return null;
    }
    ...
    try {
      ...
      const response = await sensayService.createConversation(agentUUID, targetUserID);
